...
// get dxy from V0 candidates
ROOT::VecOps::RVec<double> get_dxy(
    const FCCAnalyses::VertexingUtils::FCCAnalysesV0& SVs,
    const FCCAnalyses::VertexingUtils::FCCAnalysesVertex& PV){
    ...
    for(const FCCAnalyses::VertexingUtils::FCCAnalysesVertex& SV : SVs.vtx){
        TVector3 x_vtx(SV.vertex.position[0],
                       SV.vertex.position[1],
                       SV.vertex.position[2]);
...
// get dxyz from V0 candidates
ROOT::VecOps::RVec<double> get_dxyz(
    const FCCAnalyses::VertexingUtils::FCCAnalysesV0& SVs,
    const FCCAnalyses::VertexingUtils::FCCAnalysesVertex& PV){
...
// get chi2 from V0 candidates
ROOT::VecOps::RVec<double> get_chi2(
    const FCCAnalyses::VertexingUtils::FCCAnalysesV0& SVs){

    ROOT::VecOps::RVec<double> result;
    for(const FCCAnalyses::VertexingUtils::FCCAnalysesVertex& SV : SVs.vtx){
        int nDOF = 2 * SV.ntracks - 3;
        result.push_back(nDOF * SV.vertex.chi2);
    }
    return result;
}

// get normalized chi2 from V0 candidates
ROOT::VecOps::RVec<double> get_chi2Normalized(
    const FCCAnalyses::VertexingUtils::FCCAnalysesV0& SVs){

    ROOT::VecOps::RVec<double> result;
    for(const FCCAnalyses::VertexingUtils::FCCAnalysesVertex& SV : SVs.vtx){
        result.push_back(SV.vertex.chi2);
    }
    return result;
}
